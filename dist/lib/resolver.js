...
function configureReplaceCommand(program) {
    program
        ...
        .option("-a, --alias-config-path <path>", "Relative path to tsconfig.json or jsconfig.json for alias resolution.")
        .option("-v, --verbose", "Enable verbose output for detailed logs.", false)
        .action(async (cmdOptions) => {
        const globalOptions = program.opts();
        if (!cmdOptions.aliasConfigPath) {
            console.error("Error: The --alias-config-path <path> option is required for the replace command.");
            process.exit(1);
        }
        const tsConfigPath = path_1.default.join(globalOptions.rootPath, cmdOptions.aliasConfigPath);
        ...
        for (const sourceFile of projectSourceFiles) {
            ...
            for (const importDecl of importDeclarations) {
                ...
                const fileName = path_1.default.basename(importSourceFile.getFilePath());
                if (fileName.startsWith("index.")) {
                    ...
                    for (const named of importDecl.getNamedImports()) {
                        ...
                        if (originalFilePath === barrelFilePath)
                            continue;
                        // Calculate the relative path from the current file to the original source
                        let relativePath = path_1.default.relative(path_1.default.dirname(sourceFile.getFilePath()), originalFilePath);
                        ...
                        importPath = importPath.split(path_1.default.sep).join("/");
